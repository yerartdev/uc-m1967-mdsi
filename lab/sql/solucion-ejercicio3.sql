---------------------------------------------------------------------------------------------------------
-- Resolución del Ejercicio 3 - Tema 1
-- 
...
CREATE TABLE Asignaturas (
   ...
   nombre char not null unique,
   creditos int not null,
   idpplan int not null,
   foreign key (idplan) references PlanEstudios (idplan),
   -- BONUS
   unique(nombre,idplan)
);

CREATE TABLE ProfesoresAsignaturas (
   idasignarura int not null,
   idprofesor int not null,
   foreign key (idasignarura) references Asignaturas (idasignatura),
